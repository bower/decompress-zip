...
var signatures = {
	...
	huntSize = 1024; // We will search the last 1kb for END_OF_CENTRAL_DIRECTORY
...
var getSearchBuffer = function (fd) {
...
var findEndOfDirectory = function (buffer) {
...
var readDirectory = function (recordBuffer) {
	var files = [],
		self = this,
		record = structures.readEndRecord(recordBuffer);
...
var extractUncompressed = function (file, destination) {
	...
	return mkpath(path.dirname(destination))
		.then(pipePromise.bind(null, input, destination))
		.then(function () {
...
var Zip = function (filename) {
	...
	this._p.fileBuffer = this._p.fd
		.then(getSearchBuffer.bind(this));
	this._p.directory = this._p.fileBuffer
		.then(findEndOfDirectory.bind(this))
