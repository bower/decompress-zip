...
var extractors = {
    ...
    store: function (file, destination, zip) {
        ...
        } else if (file.directoryEntry.uncompressedSize <= zip.chunkSize) {
            writer = function () {
                return zip.getBuffer(file.dataOffset, file.dataOffset + file.directoryEntry.uncompressedSize)
                .then(writeFile.bind(null, destination));
            };
        } else {
            var input = new stream.Readable();
            input.wrap(fs.createReadStream(zip.filename, {start: file.dataOffset, end: file.dataOffset + file.directoryEntry.uncompressedSize - 1}));
    ...
    deflate: function (file, destination, zip) {
        ...
        return mkdir(path.dirname(destination))
        .then(function () {
            if (file.maxSize <= zip.chunkSize) {
                return zip.getBuffer(file.dataOffset, file.dataOffset + file.maxSize)
                .then(inflateRaw)
                ...
                var input = new stream.Readable();
                input.wrap(fs.createReadStream(zip.filename, {start: file.dataOffset}));
